...
with tabA:
    ...
    **✅ Activation 성공 기준**  
    ...
    pv_cut = pd.cut(sf["pv"], pv_bins2, right=False)  # ← 6) 퍼널에서 재사용
    pv_order = pv_cut.cat.categories.astype(str).tolist()

    pv_tbl = (sf.groupby(pv_cut, observed=False)
              .agg(sessions=("session_id", "count"),
                   act_rate=("activation", "mean"))
              .reset_index())
    ...
    dur_tbl = (sf.groupby(dur_cut, observed=False)
               .agg(sessions=("session_id", "count"),
                    act_rate=("activation", "mean"))
               .reset_index())
    ...
    hit_tbl = (sf.groupby(hit_cut, observed=False)
               .agg(sessions=("session_id", "count"),
                    act_rate=("activation", "mean"))
               .reset_index())
    hit_tbl["hit_bin"] = hit_tbl.iloc[:, 0].astype(str)

    # 세 차트의 Y축 최대치 동일화(가독성)
    y_max = float(
        pd.concat([pv_tbl["act_rate"], dur_tbl["act_rate"], hit_tbl["act_rate"]])
        .fillna(0).max()
    ) * 1.05 or 0.05  # 데이터가 거의 0이면 최소 여백
    ...
    chart_pv = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("pv_bin:N", title="PV 구간", sort=pv_order,
                    axis=alt.Axis(labelAngle=-45)),
            y=alt.Y("act_rate:Q", title="Activation Rate",
    ...
    chart_dur = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("dur_bin:N", title="지속시간(초) 구간", sort=dur_order,
                    axis=alt.Axis(labelAngle=-45)),
            y=alt.Y("act_rate:Q", title="Activation Rate",
                    axis=alt.Axis(format="~%"),
                    scale=alt.Scale(domain=[0, y_max])),
    ...
    chart_hit = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("hit_bin:N", title="Hits 구간", sort=hit_order,
                    axis=alt.Axis(labelAngle=-45)),
            y=alt.Y("act_rate:Q", title="Activation Rate",
                    axis=alt.Axis(format="~%"),
                    scale=alt.Scale(domain=[0, y_max])),
...
with tabAcq:
    ...
    st.dataframe(quality_summary[
                     ["channelGrouping", "avg_pageviews", "bounce_score", "avg_time_on_site", "transaction_rate",
                      "final_score"]])
